...
/**
  ...
  *
  * @param inputId              `inputId` provided in the request object.
  * @param inputIndex           The order in which the request object was submitted with the others (0 if alone)
  * @param candidateIndex       A request object can match multiple valid addresses. This ties the candidates to the input index
  * @param addressee            Intended recipient at an address
  * @param deliveryLine1        The first delivery line (usually the street address). This can include any of the following:
  *                           - Primary Number
  *                           - Street Name
  *                           - Street Predirection
  ...
  *                           - PMB Designator
  *                           - PMB Number
  * @param deliveryLine2        The second delivery line (if needed). It is common for this field to remain empty, but it may contain a private mailbox number
  * @param lastLine             City, state, and ZIP Code combined
  * @param deliveryPointBarcode 12-digit POSTNET™ barcode; consists of 5-digit ZIP Code, 4-digit add-on code, 2-digit delivery point, and 1-digit check digit
  * @param components           Address components, see [[Components]]
  * @param metadata             Response metadata, see [[Metadata]]
  * @param analysis             Analytics data, see [[Analysis]]
  *
...
/**
  ...
  *
  * @param urbanization             Primarily for Puerto Rican addresses; a very important component which contains area, sector, or development within a geographic area; should be included after the name of the recipient
  * @param primaryNumber            The house, PO Box, or building number
  * @param streetName               The name of the street
  * @param streetPredirection       Directional information before a street name (N, SW, etc.)
  * @param streetPostdirection      Directional information after a street name (N, SW, etc.)
  * @param streetSuffix             Abbreviated value describing the street (St, Ave, Blvd, etc.)
  * @param secondaryNumber          Apartment or suite number, if any
  * @param secondaryDesignator      Describes location within a complex/building (Ste, Apt, etc.)
  * @param extraSecondaryNumber     Descriptive information about the location of a building within a campus
  * @param extraSecondaryDesignator Description of the location type within a campus
  * @param pmbDesignator            Private mailbox unit designator (assigned by a CMRA)
  * @param pmbNumber                The private mailbox number, assigned by a CMRA
  * @param cityName                 The USPS-preferred city name for this particular address, or an acceptable alternate if provided by the user
  * @param defaultCityName          The default city name for this 5-digit ZIP Code
  * @param stateAbbreviation        The two-letter state abbreviation
  * @param zipcode                  The 5-digit ZIP Code
  * @param plus4Code                The 4-digit add-on code (more specific than 5-digit ZIP)
  * @param deliveryPoint            The last two digits of the house/box number, unless an "H" record is matched, in which case this is the secondary unit number representing the delivery point information to form the delivery point barcode (DPBC)
  * @param deliveryPointCheckDigit  Correction character, or check digit, for the 11-digit barcode
  *
  * @author Yan Doroshenko
  */
...
/**
  ...
  *
  * @param recordType               Indicates the type of record that was matched. Only given if a DPV match is made. See [[RecordType]]
  * @param zipType                  Indicates the type of the ZIP Code for the address that was matched. Only given if a 5-digit match is made. See [[ZipType]]
  * @param countyFips               The 5-digit county FIPS (Federal Information Processing Standards) code. It is a combination of a 2-digit state FIPS code and a 3-digit county code assigned by the NIST (National Institute of Standards and Technology).
  * @param countyName               The name of the county the address is in
  * @param ewsMatch                 Early warning system flag; a positive result indicates the street of the address is not yet ready for mail delivery and that the address will soon be added to the master ZIP+4 file in the coming weeks or months. This commonly occurs for new streets or streets undergoing a name change.
  *
  *    - '''true''' — The address was flagged by EWS, preventing a ZIP+4 match
  *    - '''[empty]''' — Address was not flagged by EWS
  * @param carrierRoute             The postal carrier route for the address
  * @param congressionalDistrict    The congressional district to which the address belongs. Output will be two digits from 01 - 53 or "AL." "AL" means that the entire state (or territory) is covered by a single congressional district. These include Alaska, Delaware, Montana, North Dakota, South Dakota, Vermont, Wyoming, Washington DC, Virgin Islands, and other territories
  * @param buildingDefaultIndicator Indicates whether the address is the "default" address for a building; for example, the main lobby. See [[util.Utils.CharBoolean]]
  * @param rdi                      Residential Delivery Indicator (residential or commercial). See [[ResidentialDeliveryIndicator]]
  * @param elotSequence             [[https://ribbs.usps.gov/index.cfm?page=elot eLOT]] (Enhanced Line of Travel) 4-digit sequence number
  * @param elotSort                 [[https://ribbs.usps.gov/index.cfm?page=elot eLOT]] (Enhanced Line of Travel) product was developed to give mailers the ability to sort their mailings by line of travel sequence. See [[ELOTSort]]. Empty means address was not submitted for eLOT
  * @param latitude                 The horizontal component used for geographic positioning. Empty for military or diplomatic addresses
  * @param longitude                The vertical component used for geographic positioning. Empty for military or diplomatic addresses
  * @param precision                Indicates the precision of the latitude and longitude values. See [[Precision]]
  *
  ...
  * @param timeZone                 Indicates the common name of the time zone associated with the address
  * @param utcOffset                Indicates the number of hours the time zone is offset from Universal Time Coordinated (UTC). Empty for military or diplomatic addresses
  * @param dst                      Indicates whether the timezone uses daylight savings time
...
object Precision extends Enumeration {
  ...
  implicit val f: Format[Value] = new Format[Value] {
    ...
    override def reads(json: JsValue): JsResult[Value] =
      JsSuccess(values.find(_.code == json.as[String]).get)
  }
}
...
/**
  ...
  *
  * @param dpvMatchCode      Status of the Delivery Point Validation (DPV). This indicates whether the USPS delivers mail to the address. Empty means the address was not submitted for DPV. See [[DPVMatchCode]]
  * @param dpvFootnotes      Indicates why the address was given its DPV value and potentially the type of ZIP Code that was matched. See [[DPVFootnotes]]
  * @param dpvCmra           Indicates whether the address is associated with a Commercial Mail Receiving Agency (CMRA), also known as a private mailbox (PMB) operator. A CMRA is a business through which USPS mail may be sent or received, for example the UPS Store and Mailboxes Etc.
  *
  ...
  * @param dpvVacant         Indicates that a delivery point was active in the past but is currently vacant (in most cases, unoccupied over 90 days) and is not receiving deliveries. This status is often obtained when mail receptacles aren't being emptied and are filling up, so mail is held at the post office for a certain number of days before the delivery point is marked vacant
  *
  *                       - '''Y''' — Address is vacant
  *                       - '''N''' — Address is not vacant
  *                       - '''[empty]''' — Address was not submitted for vacancy verification
  * @param dpvActive         Indicates whether the address is active, or "in-service" according to the USPS
  *
  *                       - '''true''' — Address is active
  *                       - '''false''' — Address is inactive
  *                       - '''[empty]''' — Activity status is not known by the USPS
  * @param footnotes         Footnotes of the changes made to the input address. See [[Footnotes]]
  * @param lacslinkCode      The reason for the LACSLink indication that was given. See [[LACSLinkCode]]
  * @param lacslinkIndicator Indicates whether there is an address match in the LACSLink database. See [[LACSLinkIndicator]]
  * @param suitelinkMatch    Indicates a match (or not) to the USPS SuiteLink data. SuiteLink attempts to provide secondary information such as "suite" or "apartment" whenever there is a match based on address and Firm Name (Company) inpu
  *
  * @author Yan Doroshenko
  */
case class Analysis(
...
/**
  ...
  *     - '''SecondaryDropped''' — Confirmed By Dropping Secondary; address was DPV confirmed by dropping all or part of the secondary info (apartment, suite, etc.).
  *     - '''SecondaryMissing''' — Confirmed - Missing Secondary Info; the address was DPV confirmed, but it is missing secondary information (apartment, suite, etc.).
  *
  * @author Yan Doroshenko
  */
...
/**
  ...
  *
  * @param footnotes List of DPV footnotes for a given response
  *
  * @author Yan Doroshenko
  */
...
object DPVFootnotes {
  implicit val f: Format[DPVFootnotes] = new Format[DPVFootnotes] {
    override def writes(o: DPVFootnotes): JsValue = JsString(o.footnotes.map(_.code).mkString)

    override def reads(json: JsValue): JsResult[DPVFootnotes] =
      JsSuccess(DPVFootnotes(json.as[String].sliding(2, 2).map { v => DPVFootnote(v) }.toSeq))
  }
}
...
/**
  ...
  *      - '''SecondaryMissing''' — Confirmed with missing secondary information; address is valid but it also needs a secondary number (apartment, suite, etc.)
...
/**
  ...
  *      - '''NoNewAddress''' — Match: No new address. LACSLink matched an input address to an old address which is a "high-rise default" address; no new address was provided
...
/**
  ...
  *   - '''SecondaryNumberDropped''' — LACS record - secondary number dropped; the record is a ZIP+4 street level or high-rise match. The input record matched a master file record, but the input address had a secondary number and the master file record did not
...
/**
  ...
  *
  * @param footnotes List of footnotes for a given response
  *
  * @author Yan Doroshenko
  */
...
object Footnotes {
  implicit val f: Format[Footnotes] = new Format[Footnotes] {
...
/**
  ...
  *      - '''CorrectedZipCode''' — The address was found to have a different 5-digit ZIP Code than given in the submitted list. The correct ZIP Code is shown in the ZIP Code field
  *      - '''FixedCityStateSpelling''' — The spelling of the city name and/or state abbreviation in the submitted address was found to be different than the standard spelling. The standard spelling of the city name and state abbreviation is shown in the City and State fields
  *      - '''InvalidCityStateZip''' — The ZIP Code in the submitted address could not be found because neither a valid city and state, nor valid 5-digit ZIP Code was present. SmartyStreets recommends that the customer check the accuracy of the submitted address
  *      - '''NoPlus4Assigned''' — This is a record listed by the United States Postal Service as a non-deliverable location. SmartyStreets recommends that the customer check the accuracy of the submitted address
  *      - '''SameZipForMultiple''' — Multiple records were returned, but each shares the same 5-digit ZIP Code
  *      - '''AddressNotFound''' — The address, exactly as submitted, could not be found in the city, state, or ZIP Code provided. Many factors contribute to this; either the primary number is missing, the street is missing, or the street is too horribly misspelled to understand
  *      - '''UsedFirmData''' — Information in the firm line was determined to be a part of the address. It was moved out of the firm line and incorporated into the address line
  ...
  *      - '''IncorrectAddressData''' — More than one ZIP+4 Code was found to satisfy the address as submitted. The submitted address did not contain sufficiently complete or correct data to determine a single ZIP+4 Code. SmartyStreets recommends that the customer check the accuracy and completeness of the submitted address. For example, a street may have a similar address at both the north and south ends of the street
  *      - '''DualAddress''' — The input contained two addresses. For example: 123 MAIN ST PO BOX 99
  *      - '''CardinalRuleMatch''' — Although the address as submitted is not valid, we were able to find a match by changing the cardinal direction (North, South, East, West). The cardinal direction we used to find a match is found in the components
  *      - '''ChangedAddressComponent''' — An address component (i.e., directional or suffix only) was added, changed, or deleted in order to achieve a match
  *      - '''FlaggedForLACSLink''' — The input address matched a record that was LACS-indicated, that was submitted to LACSLink for processing. This does not mean that the address was converted; it only means that the address was submitted to LACSLink because the input address had the LACS indicator set
  *      - '''FixedStreetSpelling''' — The spelling of the street name was changed in order to achieve a match
  *      - '''FixedAbbreviations''' — The delivery address was standardized. For example, if STREET was in the delivery address, SmartyStreets will return ST as its standard spelling
  *      - '''MultiplePlus4''' — More than one ZIP+4 Code was found to satisfy the address as submitted. The lowest ZIP+4 add-on may be used to break the tie between the records
  *      - '''BetterAddressExists''' — The delivery address is matchable, but it is known by another (preferred) name. For example, in New York, NY, AVENUE OF THE AMERICAS is also known as 6TH AVE. An inquiry using a delivery address of 39 6th Avenue would be flagged with Footnote P
  *      - '''UniqueZipMatch''' — Match to an address with a unique ZIP Code
  *      - '''EWSMatchSoon''' — The delivery address is not yet matchable, but the Early Warning System file indicates that an exact match will be available soon
  *      - '''BadSecondaryAddress''' — The secondary information (apartment, suite, etc.) does not match that on the national ZIP+4 file. The secondary information, although present on the input address, was not valid in the range found on the national ZIP+4 file
  *      - '''MultipleResponse''' — The search resulted in a single response; however, the record matched was flagged as having magnet street syndrome, and the input street name components (pre-directional, primary street name, post-directional, and suffix) did not exactly match those of the record. A "magnet street" is one having a primary street name that is also a suffix or directional word, having either a post-directional or a suffix (i.e., 2220 PARK MEMPHIS TN logically matches to a ZIP+4 record 2200-2258 PARK AVE MEMPHIS TN 38114-6610), but the input address lacks the suffix "AVE" which is present on the ZIP+4 record. The primary street name "PARK" is a suffix word. The record has either a suffix or a post-directional present. Therefore, in accordance with CASS requirements, a ZIP+4 Code must not be returned. The multiple response return code is given since a "no match" would prevent the best candidate
  *      - '''UnofficialPostOfficeName''' — The city or post office name in the submitted address is not recognized by the United States Postal Service as an official last line name (preferred city name), and is not acceptable as an alternate name. The preferred city name is included in the City field
  *      - '''UnverifiableCityState''' — The city and state in the submitted address could not be verified as corresponding to the given 5-digit ZIP Code. This comment does not necessarily denote an error; however, SmartyStreets recommends that the customer check the accuracy of the city and state in the submitted address
  *      - '''InvalidDeliveryAddress''' — The input address record contains a delivery address other than a PO Box, General Delivery, or Postmaster 5-digit ZIP Code that is identified as a "small town default." The USPS does not provide street delivery service for this ZIP Code. The USPS requires the use of a PO Box, General Delivery, or Postmaster for delivery within this ZIP Code
  ...
  *      - '''MatchedWithZipMove''' — The ZIPMOVE product shows which ZIP+4 records have moved from one ZIP Code to another. If an input address matches a ZIP+4 record which the ZIPMOVE product indicates has moved, the search is performed again in the new ZIP Code
  *
  * @author Yan Doroshenko
  */
